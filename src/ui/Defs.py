...
class SCREEN_BOX:
    ...
    格式：(原始宽, 原始高, x0, y0, x1, y1)。运行期按实际分辨率自动缩放，
    所以基准是多少都不影响正确性。判据的搜索框在 `DISCRIMINATORS` 里。
    """

    # ---- 委托流程 ----
    REWARD_DRAG_AREA = (2560, 1440, 69, 969, 2498, 1331)       # start_mission 鼠标安全区
    # 轮次数字 OCR：新版 HUD 在左上角，数字在 x172-190 / y245-268
    ROUND_INFO_OCR = (1600, 900, 170, 240, 230, 275)
    SETTING_OTHER_TAB = (1600, 900, 500, 0, 620, 60)            # 设置「其他」页签
    # 「复位角色」：深色条 + 浅灰文字，没有可裁图形，整条都是可点范围。
    # 同时兼作点击前的鼠标安全范围（框架里这是闸门，鼠标不在框内就不做安全移动）
    RESET_CHARACTER_AREA = (1600, 900, 1166, 606, 1469, 642)
    RESET_OK_SAFE_BOX = (2560, 1440, 1298, 772, 1735, 846)      # 点「确定」前鼠标允许停留的范围

    # ---- HUD（新截图 1600×900）----
    SERUM_ICON = (1600, 900, 18, 322, 68, 394)                  # 血清水滴图标
    ULTIMATE_KEY_ICON = (1600, 900, 1404, 810, 1474, 858)       # 右下角 Q 终结技键

    # ---- 密函选择 ----
    # 拖拽/鼠标安全区：相对框 (0.4432,0.3556)-(0.9750,0.6037)，按 1600×900 展开
    LETTER_DRAG_AREA = (1600, 900, 709, 320, 1560, 543)

    # ---- 大世界 / 追踪 ----
    FIND_TRACK_POINT = (2560, 1440, 454, 265, 2110, 1094)       # track_point 搜索区
    HEDGE_PROCESS_INFO = (3840, 2160, 12, 494, 637, 1002)       # 避险进度信息
    HEDGE_TRACK_POINT = (2560, 1440, 2183, 82, 2414, 140)       # 避险右上角追踪点

    # ---- 勘察 / 护送 ----
    HEALTH_BAR = (2560, 1440, 91, 512, 303, 525)                # 勘察目标血条
    ESCORT_TRACK_POINT = (2560, 1440, 850, 360, 1710, 1080)     # 护送追踪点
    ESCORT_TARGET_HP = (1920, 1080, 38, 401, 284, 426)          # 护送目标血条

    # ---- 范围外（钓鱼 / 迷宫 / 肉鸽 / 轮盘），保留原作者基准，待重做 ----
    FISH_CHANCE = (3840, 2160, 3269, 1672, 3505, 1908)
    MAZE_MECH_RETRY_A = (2560, 1440, 2287, 1006, 2414, 1132)
    MAZE_MECH_RETRY_B = (3840, 2160, 3367, 1632, 3548, 1811)
    MAZE_PUZZLE = (3840, 2160, 2336, 604, 3307, 1578)
    ROGUE_DIALOG = (2560, 1440, 1504, 854, 1555, 1224)
    ROGUE_SPACE_TEXT = (2560, 1440, 2092, 1380, 2183, 1418)
    ROULETTE_SPACE_TEXT = (2560, 1440, 1878, 736, 1963, 769)
    ROULETTE_F_SEARCH = (2560, 1440, 2275, 1235, 2365, 1315)

    # ---- 全屏 ----
    FULL_SCREEN = (2560, 1440, 1, 1, 2559, 1439)
...
class COORD:
    ...
    # ---- 密函选择 ----
    LETTER_NOT_USE = (763, 390)               # 第 1 槽「⊘ 不使用」卡面中心
    # ⊘ 槽实测 x720-805 / y338-452，取卡面中心附近，避免随机点落到相邻卡上
    LETTER_NOT_USE_SAFE = (740, 360, 790, 420)
