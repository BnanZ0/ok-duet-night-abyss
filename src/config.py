...
sensitivity_config_option = ConfigOption(
...
afk_config_option = ConfigOption(
    "挂机设置",
    {  # 全局配置示例
        "提示音": True,
        "提示音次数": 2,
...
monthly_card_config_option = ConfigOption(
    "Monthly Card Config",
    {"Check Monthly Card": False, "Monthly Card Time": 5},
...
config = {
    ...
    "my_app": [
        "src.globals",
        "Globals",
    ],  # 全局单例对象, 可以存放加载的模型, 使用og.my_app调用
